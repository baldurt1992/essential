<template>
    <article :class="['essential-plan-card', {
        'essential-plan-card--recommended': isRecommended,
        'essential-plan-card--current': isCurrent
    }]">
        <header class="plan-card__header">
            <div v-if="isCurrent" class="essential-plan-badge essential-plan-badge--current">
                ACTUAL
            </div>
            <div v-else-if="highlight" class="essential-plan-badge">
                {{ highlight }}
            </div>
        ...
        <div v-if="isCurrent && activeSubscription"
        ...
        <div class="plan-card__cta">
            <button v-if="isCurrent" type="button" disabled
                class="essential-contact-cta essential-contact-cta--ghost plan-card__cta-button">
                Plan Actual
            </button>
            <button v-else-if="currentPlanUuid && canUpgrade" type="button"
                :disabled="isCreatingCheckout === plan.id" @click="$emit('checkout', plan, $event)"
                class="essential-contact-cta essential-contact-cta--primary plan-card__cta-button">
            ...
            <button v-else type="button" :disabled="isCreatingCheckout === plan.id"
                @click="$emit('checkout', plan, $event)"
                class="essential-contact-cta essential-contact-cta--primary plan-card__cta-button">
                <span v-if="isCreatingCheckout === plan.id">Creando checkout…</span>
